...
class EchoDynamicDataset(torchvision.datasets.VisionDataset):
    ...
    def _get_video_clip(self, video, video_frame_real_mask, period_this, start_index, largeTrace=None, smallTrace=None):
        frame_idx_fn = lambda s: s + period_this * torch.arange(self.length)
        frame_indices = frame_idx_fn(start_index)

        video = video[:, frame_indices, :, :]
        video_frame_real_mask = video_frame_real_mask[frame_indices]
        ...
        if self.pad is not None and self.pad > 0:
            ...
            if largeTrace is not None:
                ...
                largeTrace = largeTrace + torch.tensor([j, i])
            ...
            if smallTrace is not None:
                ...
                assert hh == h and ww == w
                smallTrace = smallTrace + torch.tensor([j, i])
        ...
        return video, frame_indices, video_frame_real_mask, largeTrace, smallTrace
    ...
    def __getitem__(self, index):
        ...
        if f < self.length * period_this + self.time_pad:
            # zero padding
            # video = torch.cat([video, torch.zeros((c, self.length * period_this + self.time_pad - f, h, w), dtype=video.dtype)], dim=1)
            
            # repeat padding
            repeat_time, repeat_tail = (self.length * period_this + self.time_pad) // f, (self.length * period_this + self.time_pad) % f
            video = torch.cat([video] * repeat_time + [video[:, :repeat_tail, ...]], dim=1)
            
            video_frame_real_mask = torch.cat((video_frame_real_mask, torch.zeros(self.length * period_this + self.time_pad - f, dtype=video_frame_real_mask.dtype)), dim=0)
            c, f, h, w = video.shape  # f == self.length * period_this + self.time_pad
        
        if self.val_test_clips > 1 and not self.train:
            remainder_for_start = f - (self.length - 1) * period_this
            start_index_list = torch.linspace(0, remainder_for_start - 1, self.val_test_clips, dtype=torch.int)
            video_clip_list = []
            video_clip_frame_indices_list = []
            video_clip_frame_real_mask_list = []
            for i, start_index in enumerate(start_index_list):
                video_clip, video_clip_frame_indices, video_clip_frame_real_mask, _, _ = self._get_video_clip(video, video_frame_real_mask, period_this, start_index)
                video_clip_list.append(video_clip)
                video_clip_frame_indices_list.append(video_clip_frame_indices)
                video_clip_frame_real_mask_list.append(video_clip_frame_real_mask)
            video_clip = torch.stack(video_clip_list, dim=0)  # [clips, C, L, H, W]
            video_clip_frame_indices = torch.stack(video_clip_frame_indices_list, dim=0)  # [clips, L]
            video_clip_frame_real_mask = torch.stack(video_clip_frame_real_mask_list, dim=0)  # [clips, L]
            return {
                'inputs': {
                    'x': torch.as_tensor(video_clip),  # [clips, C, L, H, W]
                    'video_frame_real_mask': torch.as_tensor(video_clip_frame_real_mask),  # [clips, L]
                },
                'targets': {
                    **{'gt_' + k.lower(): torch.as_tensor(v) for k, v in target.items()},
                    ...
                    'frame_indices': torch.as_tensor(video_clip_frame_indices),  # [clips, L]
                }
            }
        ...
        video_clip, video_clip_frame_indices, video_clip_frame_real_mask, largeTrace, smallTrace = self._get_video_clip(
            video,
            video_frame_real_mask,
            period_this,
            start_index,
            getattr(target, 'LargeTrace', None),
            getattr(target, 'SmallTrace', None)
            )
        if getattr(target, 'LargeTrace', None) is not None:
            target['LargeTrace'] = largeTrace
        if getattr(target, 'SmallTrace', None) is not None:
            target['SmallTrace'] = smallTrace
        ...
        return {
            'inputs': {
                ...
                'video_frame_real_mask': torch.as_tensor(video_clip_frame_real_mask),
            },
            'targets': {
                **{'gt_' + k.lower(): torch.as_tensor(v) for k, v in target.items()},
                'file_name': video_file_name.split('.')[0],  # no file suffix
                'index': torch.tensor(index),
                # 'video_frame_real_mask': torch.as_tensor(video_frame_real_mask),
                'frame_indices': torch.as_tensor(video_clip_frame_indices),
            }
        }
...
@data_module_register('echo_dynamic')
